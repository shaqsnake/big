...
**方案：** 增加 `big impact <version> [--depth N] [--verbose] [--full]`。命令先校验目标 version read 权限，再按 `provenance_edges.upstream_version_id` 查询下游 consumes 边。默认 `--depth 1` 只显示直接下游；增加 depth 时按层递归展开，并用 visited set 防止环或重复节点。输出区分 visible 与 restricted downstream，避免把无权限节点误报为无影响。`--verbose` 展示 evidence path，`--full` 进一步展开 upstream manifest 和 evidence FileRef 摘要。
...
- `--verbose` 展示 edge evidence 摘要，例如 CLI 提供的 evidence path；`--full` 进一步展示 evidence manifest、FileRef role/path/hash/size 和原始 evidence JSON。
...
- Given 用户使用 `--full`
- When evidence path 可以在 upstream version 的 FileRef 中找到
- Then 输出 evidence manifest 与 evidence FileRef 摘要。
...
- `python -m pytest`
- `python -m pytest tests/test_cli_prototype.py::test_impact_lists_direct_and_recursive_consumes`
