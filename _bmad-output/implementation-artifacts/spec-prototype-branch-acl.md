...
- Given 当前用户位于无 read 权限的 workspace 目录中
- When 用户执行 `big status`
- Then 系统仍显示 repo/workspace 上下文，但把 head 详情显示为 `restricted`，不输出 version id、step、state 或 message。
