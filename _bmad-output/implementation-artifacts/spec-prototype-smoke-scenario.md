...
**问题：** `make smoke` 原先只覆盖 init、commit、log，已经落后于当前原型的关键路径；用户在 WSL 中手工验证时，还需要确认 shell 集成输出、workspace-private ref 隔离、parent-chain lineage、Candidate 晋升、命名 branch、branch checkout、`--print-path` 输出模式、历史版本 `--new-branch` checkout、recipe_only inputs-only checkout、仓库级完整性校验、repo stats 和 audit hash-chain。
...
- And repo stats 的 review 分布包含 `Candidate: versions=1` 和 `Exploring: versions=1`，retention 分布包含 `resident: versions=1` 和 `recipe_only: versions=1`。

- Given smoke 完成
- When 执行 `big audit verify`
- Then 输出 `events: 7` 和 `integrity: ok`。
