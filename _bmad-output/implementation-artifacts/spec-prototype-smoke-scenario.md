...
**问题：** `make smoke` 原先只覆盖 init、commit、log，已经落后于当前原型的关键路径；用户在 WSL 中手工验证时，还需要确认 shell 集成输出、workspace-private ref 隔离、parent-chain lineage、Candidate 晋升、命名 branch、branch checkout、`--print-path` 输出模式、历史版本 `--new-branch` checkout、仓库级完整性校验和 repo stats。
