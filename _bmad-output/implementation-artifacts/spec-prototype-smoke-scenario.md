...
**问题：** `make smoke` 原先只覆盖 init、commit、log，已经落后于当前原型的关键路径；用户在 WSL 中手工验证时，还需要确认 shell 集成输出、workspace-private ref 隔离、命名 branch、branch checkout、`--print-path` 输出模式、历史版本 `--new-branch` checkout 和 repo stats。
...
- When smoke 执行 shell-init、commit、branch create、branch checkout、`--print-path`、历史版本 `--new-branch` checkout
...
- Given smoke 完成
- When 查看 repo stats
- Then 输出 2 个 version、10 个 file_ref、4 个唯一引用对象和 4 个 CAS 对象。
