...
**方案：** 增加本地 `audit_events` 表。每个写操作在同一 SQLite 事务中追加一条 audit event，记录 action、entity、actor、created_at、payload 摘要、previous_hash 和 event_hash。`event_hash` 基于上一条 hash 与当前事件规范 JSON 计算。增加 `big audit log` 查看最近事件，增加 `big audit verify` 顺序重算 hash-chain 并检测断链或事件篡改。

## Boundaries

- 覆盖当前原型中的写操作：`commit`、`branch create`、`reset`、`promote`。
- audit payload 只保存摘要字段，不保存完整 FileRef 列表，避免百万文件 commit 让审计表膨胀。
- 本切片不实现外部不可变介质锚定、不实现导出、不实现服务端 append-only 存储，也不接 Linux groups 权限上下文。
