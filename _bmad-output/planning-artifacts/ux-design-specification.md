...
**作者:** shaqsnake
**日期:** 2026-05-28
**修订:** 2026-06-02，依据EDA进程隔离、低版本NAS和GUI解耦要求修正稳定分支目录、显式restore、Candidate交付与公共API语义

---

## UX Revision Baseline (2026-06-02)

本节覆盖文档中与其冲突的早期描述：

- 用户在 `/data/<project>/user/<username>/<branch>/` 下拥有每分支稳定目录。从用户视角，分支就是具体目录；内部登记信息不引入新的日常命令。
- `big commit` 仍是一条命令，但内部会建立 staging 快照；发布完成前禁止修改同一工作区中的提交范围。
- `big checkout <branch>`进入另一个稳定目录，不改写源目录。旧目录中的EDA进程继续使用旧分支；新目录中新启动的进程使用新分支。
- 历史版本默认用`big checkout <version> --new-branch <name>`物化为兄弟目录。仅`big restore --in-place <version>`允许在静默目录中执行可恢复增量替换。
- 检测到未提交修改或 BIG 管理的活动 lease 时，显式原地restore默认拒绝。手工启动的EDA进程无法可靠自动发现；严格受控执行使用`big run -- <command>`。
- 低版本 NAS 不支持 reflink/COW。目录物化仅使用普通copy；禁止 writable hardlink/symlink 指向只读 CAS。
- 官方GUI是可选参考客户端；其他系统可以通过相同公共API、OpenAPI schema和事件契约定制自己的GUI。
- UI 同时展示评审状态（Exploring/Candidate/Pinned/Golden）和驻留状态（resident/recipe_only/archived/missing）。

## Executive Summary

### Project Vision

BIG 是面向芯片设计的大文件版本管理与团队协作系统。核心洞察是：芯片设计的版本管理对象不是文件，而是设计决策历史。BIG 通过"制品集+文件"双中心模型，解决工程师手动命名混乱、问题追溯慢半天以上、交付依赖人工规范而非系统保障的痛点。系统采用 CLI 为核心交互界面，以版本化公共API作为集成边界；Electron+Web GUI 是用于版本树/血缘图可视化和版本挑选操作的可选参考客户端。系统部署在现有 NAS 基础设施上，追求工程师工作方式的零侵入和最少认知负荷。
...
4. **双状态表达** — 评审状态 Exploring→Candidate→Pinned→Golden 与驻留状态 resident/recipe_only/archived/missing 如何在 CLI 和 GUI 中统一表达，避免用户误以为某个历史版本一定能立即完整checkout？
...
- **commit**（提交）：工程师跑完 EDA 工具后，一条命令锁定输入、参数、输出的完整快照。这是每天重复最频繁的操作，必须快（流式staging与哈希进度明确）、必须简单（内部staging对用户透明）、必须可靠（输入完整性和文件稳定性自动校验）。
- **explore**（探索）：在版本历史和血缘图中导航——对比不同尝试的配方差异、沿着血缘链追溯决策源头、查看跨分支的依赖关系。CLI 提供快速查询，GUI 提供全局可视化。
- **checkout**（切换）：切换到另一个分支时进入它的稳定目录，旧目录完全不动；从历史版本重新出发时默认物化新的兄弟分支目录。只有显式restore才会原地替换当前目录内容。
...
| GUI (Electron+Web) | 可选参考望远镜 | ~25% | 版本树可视化、血缘图浏览、版本挑选、DSO分组概览；可被外部定制GUI替代 |
| 公共API + Python SDK | 系统集成层 | ~5% | pds_xxx、DSO系统、官方GUI和外部GUI的程序化调用 |

平台约束：
- 纯键鼠操作，无触屏需求
- CentOS NAS 环境，本质是本地磁盘性能
- CLI 必须支持 SSH 远程使用（无 X11 依赖）
- GUI 独立运行，暂不嵌入 EDA 工具环境；只使用公共API和事件契约
- CLI、SDK、官方GUI和外部GUI共享同一套概念模型和状态数据

### Effortless Interactions

1. **commit 进度明确** — staging复制与哈希计算流式进行，进度条即时反馈；工程师可继续查看日志等只读操作，但发布完成前不能修改同一提交范围
2. **checkout 安全切换** — branch checkout进入稳定兄弟目录且不改写旧目录；shell集成使日常仍是一条命令。显式原地restore才检测dirty state与受管lease，并通过恢复journal避免中断后留下不明状态
3. **血缘追溯一键到达** — `big log --lineage <id>` 直接输出决策链，无需翻目录、无需问同事
4. **评审晋升一步到位** — `big promote <id>` 一条命令完成评审阶段跳跃；驻留策略变化单独反馈
5. **文件视图降级入口** — 工程师可以完全不关注"制品集"概念，通过文件路径直接操作，制品集在背后自动管理

### Critical Success Moments

1. **第一次 commit 后发现路径没变** — "一条命令就锁定快照了，目录也不用搬。" → 低侵入的顿悟时刻，这是用户从"试一试"到"愿意用"的转折点
2. **血缘追溯 30 秒找到问题根源** — 从半天到 30 秒的落差冲击，这是用户从"愿意用"到"离不开"的转折点
3. **GUI 中一眼看清版本树全貌** — 从混乱命名到结构化全局视图的认知解放，这是用户信任系统组织能力的关键
4. **checkout 回退后从兄弟目录重新出发** — 时间旅行的自然感，回退不是"删掉重来"而是"从那个节点创建新分支继续"，derived_from 语义边让决策链完整

### Experience Principles

1. **零侵入** — 工程师仍在NAS真实目录中工作，每个分支路径稳定，EDA工具不需要任何适配。BIG是安静的幕后管理者，不是需要被照顾的系统。
...
| 安心 | 焦虑 | 数据完整性自动守护，分支路径稳定且互相隔离，Golden 数据冗余保护 |
...
- 反模式：二进制文件全量存储导致存储膨胀——BIG 先用文件级CAS和生命周期策略缓解；块级去重仅对基准验证有效的文件类型启用
...
- Perforce Workspace 视图 → `big checkout --subset`（适配 NAS 环境的用户私有分支目录）
...
如果只用一句话让工程师理解 BIG："跑完一行提交，版本就锁定；切分支只换目录，不会碰正在运行的工具。"
...
- 切分支 = 进入另一个稳定兄弟目录，旧目录内容不变，运行中的工具继续留在旧分支
- 原地恢复 = 仅通过显式restore改写当前目录；运行中写入工具需要停止，完成后重新打开文件
...
2. **分支路径稳定** — 每个分支使用稳定真实目录，工具无需适配BIG存储格式；切分支不改写旧目录，运行中进程不会混用内容
...
1. **发起** — 工程师执行 `big commit -m "message"`，无需手工 staging，工作目录即提交范围（`.bigignore` 排除规则过滤）；无参数时自动打开编辑器输入提交消息（对标 `svn commit`）
2. **交互** — 内部 staging 复制与哈希流式计算，进度条即时反馈（`Snapshotting files... ████████░░ 80% | 12.4s elapsed`）；输入完整性和文件稳定性并行校验，变化文件会触发重试或失败
3. **反馈** — 成功：`✓ Committed as a1b2c3d [Exploring/resident]` + 配方摘要 + 去重比；失败：`✗ Commit failed` + 具体原因 + 可操作建议
4. **完成** — manifest发布后工作目录状态不变；自然延伸到 `big log` / `big promote` / GUI 版本树
...
### checkout 分支切换与历史回退流程

工程师执行`big checkout <branch>`后：
1. 解析目标稳定目录 → 2. 目录不存在时从CAS普通copy物化兄弟目录 → 3. 输出目标路径 → 4. shell wrapper进入目标目录 → 5. 完成反馈（✓ + 分支 + 真实路径）。旧目录完全不动，旧EDA进程继续使用旧分支。

工程师执行`big checkout <version> --new-branch <name>`后，系统从历史manifest物化新的兄弟分支目录，再进入该目录。回退不会覆盖原目录。

仅当工程师明确执行`big restore --in-place <version>`时，才进入原地恢复流程：
1. 未提交变更检测（有变更→选择提交/暂存/丢弃/取消） → 2. 受管lease检测（活动lease→默认拒绝并提示处理；提醒停止手工启动的写入工具） → 3. 对比manifest，仅选择变化文件 → 4. 从CAS普通copy到同目录临时文件并校验 → 5. 逐文件替换并记录restore journal → 6. 全部完成后更新HEAD与generation → 7. 完成反馈（✓ + 文件数 + 大小 + “请重新打开文件/重启工具”提示）。

CLI子进程不能改变父shell的当前目录，因此一次性安装的shell wrapper是`big checkout`单命令体验的一部分。未安装wrapper时，CLI输出可直接执行的`cd -- <target-path>`。

### promote 晋升流程

PD Lead 执行 `big promote <id>` 后：
1. 显示当前→目标评审阶段 → 2. 系统校验（配方完整性/验证结果） → 3. Golden操作二次确认 → 4. 更新review_state并单独计算retention_state变化 → 5. 若进入Candidate，同事务写入可靠交付事件，流水线从不可变manifest/CAS发布版本化交付目录 → 6. 完成反馈（✓ + 阶段跃迁 + 驻留变化描述 + 交付状态） → 7. GUI Badge动画过渡
...
1. **最少步骤到达价值** — commit 一条命令完成，无需用户手工 staging/add/push；内部快照流程透明执行
...
展示评审状态从 Exploring→Candidate→Pinned→Golden 的阶段流转，并在节点旁独立展示驻留状态。当前阶段脉冲高亮；驻留状态变化单独标注。Growth 阶段实现阶段跃迁过渡动画。
...
CLI 格式：`abc1234 [Candidate/resident]`；GUI 使用 review state Badge + retention state 辅助标签。Golden 有★图标。跃迁时 GUI 0.3s Badge 过渡动画。三重传达：颜色+文字+图标。
...
可逆操作不确认，不可逆操作必须确认。Golden晋升需输入"GOLDEN"文本确认。确认默认为取消（y/N）。显式原地restore丢弃变更需选择确认。
