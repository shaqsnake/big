...
| **D. fastcdc + 自建 Pack** | 1.7.0 | 库+自建 | 块级 CDC | 需基准验证 | Growth 条件候选 |
...
| **MVP 适配度** | 完美 — 满足 27 个 MVP FR，NFR1/NFR2/NFR4 可达标 |
| **Growth 演进** | 加 fastcdc 块级层；参数文件、未压缩文本和结构稳定二进制的小修改场景适合 CDC 去重 |
| **风险** | 无 — 最简方案，零外部依赖 |
...
**参数文件的特殊收益：** TCL/YAML 参数文件的不同版本间通常只改几行，若文件规模和块参数足以形成多个稳定 chunk，FastCDC 可能只需存储变化附近的新 chunk。对很小的参数文件，收益可能主要来自文件级CAS、压缩和配方复用；对 DSO 场景（100+ case 共享大部分参数文件、只微调少量内容），应通过真实语料基准决定是否启用小块参数。

**压缩/opaque大文件风险：** FastCDC 的局部性依赖“未修改区域在字节流中仍然相同”。整体压缩、加密或工具私有数据库可能把源数据的小改动扩散成压缩后字节的大面积变化，导致块指纹无法复用。此类文件只能通过真实基准判定；若去重差，应回退到文件级CAS、生命周期淘汰和PPA排名保留。
...
| 参数文件去重 | **最优** — 文本/配置类小改动通常只新增少量chunk |
| 压缩/opaque DB | **高风险** — 整体压缩或加密后的字节流可能导致跨版本chunk复用率接近整文件级 |
...
|------|:-----------:|:-----------:|:---------:|:-------------:|:---------:|:-------:|
| NAS 直读 ≥70% | **1.0x** | ~1.0x | 1.5-3x | 需基准验证 | 网络延迟 | 网络延迟 |
...
**Growth：方案 A + 方案 D（文件级热缓存 + fastcdc 块级冷存储，仅对通过基准的文件类别启用）**
- 参数文件、脚本、未压缩文本和结构稳定二进制的微小修改场景优先验证
- 整体压缩、加密或opaque大型DB单独建档，不承诺块级去重收益
- DSO 场景（100+ case共享参数文件、劣序DB淘汰）主要收益来自参数文件复用和生命周期策略，块级去重只是条件性加成
- 基准测试数据指导FastCDC参数选择和“启用/禁用”文件类型白名单

核心优势：**MVP 低复杂度起步 → Growth 按真实基准选择性叠加 → 对CDC友好文件获得增量去重，对压缩/opaque文件保持文件级CAS退路**
...
| NFR14 | 单CAS对象/chunk不影响其他 | CAS对象独立存储；Growth chunk独立校验 | ✅ |
...
- CAS 双层演进路径仍可保留，但FastCDC只应对CDC友好文件类别启用；压缩、加密或opaque大文件必须由真实EDA基准决定是否降级为文件级CAS
