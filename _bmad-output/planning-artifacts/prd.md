...
1. **存储空间节省**：相比当前手动命名管理方式，MVP通过文件级CAS与分层存储达到 > 50% 节省；Growth阶段仅对CDC友好的文件类型追求 > 80% 节省。压缩、加密或opaque大型DB不承诺块级去重收益，主要依赖生命周期淘汰、PPA排名保留和交付归档策略
2. **交付周期缩短**：项目交付周期缩短 > 30%（来源于流水线标准化、追溯效率提升、减少人为错误返工）
3. **团队采纳率**：上线6个月内，团队 > 90% 的工程师日常使用BIG进行版本管理
4. **人为错误减少**：因选错文件或版本混乱导致的返工减少 > 70%
5. **DSO存储优化**：对接DSO系统后，自动寻优存储空间节省 > 90%（主要来自自动淘汰劣序DB和保留Top-K完整DB，而非默认依赖块级去重），寻优过程因磁盘不足导致的失败率降为0
...
|------|---------|------|------|
| 问题版本追溯耗时 | ≥ 半天 | < 30秒 | |
| 存储空间占用 | 100%（基线） | MVP: < 50%; Growth: CDC友好文件 < 20% | Growth目标依赖真实基准；压缩/opaque DB单独按生命周期策略衡量 |
...
| BIG (FastCDC，待EDA基准验证) | 可调CDC块 | 对未压缩/低熵/结构稳定文件可接近变更块大小；对整体压缩、加密或opaque DB可能接近整文件重存 |

FastCDC 适合作为 Growth 候选，但块大小、去重比例和读放大不能直接从通用数据集外推到 EDA 文件。压缩/加密文件的字节流会隐藏源数据相似性，小改动可能导致后续压缩字节大面积变化，因此不能把CDC收益作为这类文件的默认假设。MVP 必须使用真实 Innovus DB、GDS/OASIS、LEF/DEF、压缩归档和参数文件建立分类型基准，再决定 min/avg/max 参数与是否引入 Pack。
...
| 假设 | 验证方法 | 成功标准 | 失败退路 |
|------|---------|---------|---------|
| EDA文件CDC去重有效 | 收集真实EDA项目的多版本文件，按格式跑FastCDC去重测试，至少区分文本/结构化二进制/压缩或加密opaque DB | CDC友好类别去重比 ≥ 3:1；压缩/opaque类别只记录基线不设收益承诺 | 对该类别降级为文件级CAS + 生命周期淘汰 |
...
| CAS块级去重 | EDA文件去重比可能低于预期，尤其整体压缩、加密或opaque大型DB的小改动可能导致CDC块复用率接近整文件级 | 对该类别回退到文件级CAS + 生命周期/PPA淘汰；仅对CDC友好类别启用FastCDC |
...
| 风险 | 缓解措施 |
|------|---------|
| 文件级CAS在增量修改场景存储膨胀 | MVP先用分层存储（Exploring只存配方）缓解；Growth仅对CDC友好文件引入FastCDC。压缩/opaque大型DB继续依赖生命周期淘汰、PPA排名保留和归档策略 |
...
| NFR14: 故障隔离 | 单个CAS对象或Growth阶段单个chunk损坏时，系统可检测并报告，不影响其他数据 | 注入单个CAS对象/chunk损坏，验证检测告警触发、其他文件正常访问 | 局部故障不扩散 |
