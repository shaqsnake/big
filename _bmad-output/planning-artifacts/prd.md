...
BIG是一个面向芯片设计的大文件版本管理与团队协作系统，旨在让工程师从手动版本管理的混乱中解放出来，通过制品集血缘图实现秒级问题追溯，通过流水线实现规范化交付，支撑越来越复杂的芯片设计项目的迭代和交付。在自动寻优（DSO）场景下，一个模块需并行探索100+组APR参数组合以寻找最优PPA，产生的海量Innovus DB可达100T级别，频繁因磁盘空间耗尽导致寻优失败。BIG通过分层存储和智能淘汰机制，可自动保留较优设计DB、回收劣序DB空间，与DSO系统无缝对接解决存储膨胀痛点。系统采用CLI为核心交互方式，辅以Electron+Web GUI用于版本树/图可视化和版本挑选操作。BIG需部署在现有NAS基础设施上，兼容EDA工具链，最小化工程师的学习成本和工作方式变更。

### What Makes This Special

BIG的核心洞察是：**芯片设计的版本管理对象不是文件，而是设计决策历史。** 基于此洞察，BIG区别于所有现有工具的四个维度：制品集+文件双中心模型、CAS块级去重存储、配方即缓存Key、流水线驱动交付。详见 Innovation & Novel Patterns 章节。
...
2. **存储可靠性**：Golden阶段数据零丢失，整体数据丢失率 < 0.01%
...
- 当前团队通过自动化流程系统统一创建规范的目录结构，BIG的"一分支一目录"模型需要适配或迁移这种结构约定（不能破坏现有的目录规范）
- EDA工具对输入文件的路径依赖是BIG设计的关键约束——版本切换必须保证工具看到的路径不变
...
这是BIG最根本的创新点，也是针对芯片场景的独特洞察。现有版本管理系统的中心原语要么是commit（Git），要么是文件版本（Perforce），要么是Run/Execution（MLflow/MLMD），没有任何系统把"一组配套的输入文件+工具参数+输出文件"作为原子版本单位。
...
| BIG (FastCDC) | 256KB块 | ~100MB + 少量新块 |

推荐参数（256KB avg）在SSD上读放大仅1.3-1.5x，去重质量很好，是实践中的最佳平衡点。64KB虽然去重更细但读放大达2.5-3x，不适合性能敏感场景。

**3. 配方即缓存Key（系统级优化）**

DVC的Run Cache和Pachyderm的Datum Caching实现了类似概念，但应用范围有限。BIG可以将其系统化：
...
| NAS直读性能可达标 | CAS拼接 + symlink/hardlink方案基准测试 | 读吞吐 ≥ NAS直读的70% | 热路径文件预物化到本地SSD缓存 |
...
| 配方即缓存Key | 变参探索场景命中率低 | 缓存是增量收益，不命中不影响功能正确性 |
...
BIG是一个面向芯片设计工程师的CLI+GUI开发者工具，采用Python为主要开发语言（MVP阶段），CLI为核心交互界面，Electron+Web为GUI可视化界面。部署目标为CentOS生产环境，编译安装方式。BIG需与现有Python+TCL自动化流程系统双向集成，流水线引擎定位为子进程编排器。

### Language & Runtime

- **主要语言：** Python（MVP阶段优先跑通，性能瓶颈后续用C或Rust扩展优化）
- **GUI技术栈：** Electron + Web（独立运行环境，原型阶段先不整合到现有EDA环境）
...
- **CLI命令体系：** 推动"最少概念、最少步骤、最少认知负荷"原则（详见 Domain-Specific Requirements）
  - 版本管理核心：`init`, `commit`, `checkout`, `log`, `branch`
  - 流水线：`pipeline run`, `pipeline status`
  - 仓库管理：`repo init`, `repo config`, `repo stats`, `repo gc`
  - 归档：`archive export`
- **Python集成接口：** 供现有 `pds_xxx` 命令和未来扩展调用的Python API
- **GUI功能范围（原型阶段）：** 版本树可视化、版本挑选操作；独立运行，暂不嵌入EDA环境
...
print(f"Committed: {artifact.id} [{artifact.lifecycle}]")

# 血缘追溯
lineage = big.log.lineage(artifact_id="signoff_v5", direction="upstream")
for node in lineage:
    print(f"  {node.id} [{node.lifecycle}] <- {node.param_changes}")
```
...
big checkout fp_v2  # 回退到早期版本重新出发
```

### Migration & Onboarding

- **目录结构迁移：** 需要支持从现有自动化流程系统的目录结构迁移到BIG的"一分支一目录"模型
...
| 现有自动化流程目录 | BIG目录 | 说明 |
|------------------|--------|------|
| `/proj/3dic/user/liming/fp_v1` | `/proj/3dic/branches/fp-exploration` | 一分支一目录，版本通过commit管理 |
| `/proj/3dic/user/liming/fp_v2` | 同上（同一分支的不同commit） | 无需新建目录，checkout切换 |
| `/proj/3dic/release/golden_v5` | `/proj/3dic/branches/main` + Golden标记 | Golden由生命周期阶段表达 |
...
| `rm -rf fp_v1 && cp -r fp_v2 fp_v1` | `big checkout <id>` | 原子切换替代手动覆盖 |
...
- FR6 [MVP]: 工程师可以将工作区切换到指定制品集版本（checkout，保证文件路径不变）
- FR7 [MVP]: 系统可以为每个制品集版本计算并记录配方哈希（hash(输入+参数)）

### 分支管理

- FR8 [MVP]: PD Lead可以在项目中创建命名分支
- FR9 [MVP]: 工程师可以在分支间切换工作区
- FR10 [MVP]: 工程师可以在NAS上以独立目录访问各分支（一分支一目录）
- FR11 [MVP]: 工程师可以在切换分支时保持文件路径结构不变（EDA工具无感知）
...
- FR19 [MVP]: 系统可以根据制品集的生命周期阶段自动分层存储（Exploring→Candidate→Pinned→Golden）
- FR20 [MVP]: 系统可以在Exploring阶段只保留制品集的配方数据而不保留输出文件
...
- FR30 [Growth]: 外部系统可以通过BIG的Python API调用BIG的版本管理操作
...
- FR38 [Growth]: 系统可以根据PPA加权得分排名驱动存储淘汰策略——保留Top-K设计的完整DB，劣序DB降级为Exploring只保留配方+PPA指标。PPA得分由DSO系统计算并随制品集提交，BIG作为淘汰决策输入，不负责PPA计算
...
| NFR5: 版本切换延迟 | checkout操作完成延迟 < 同等文件量cp操作的2倍 | 对比big checkout与cp -r在相同文件集（含10万级别文件）的耗时，取5次中位值 | 一分支一目录+CAS拼接应接近文件拷贝速度 |
| NFR6: 并发支持 | 支持同一分支上10个工程师同时commit不冲突 | 10个客户端同时对同一分支执行commit，验证无数据损坏和丢失，连续测试100轮 | 10-30人团队的真实场景 |
...
| NFR8: 审计追踪 | 所有写操作（commit、晋升、权限变更）记录审计日志，不可篡改 | 执行写操作后检查审计日志完整性；尝试篡改日志文件验证被拒绝 | Golden晋升的可追溯性要求 |
...
| NFR | 指标 | 测量方法 | 依据 |
|-----|------|---------|------|
| NFR11: Golden数据零丢失 | Golden阶段制品集数据丢失率 = 0 | 模拟单盘/单节点故障，验证所有Golden数据仍可完整读取；修复后验证副本同步 | Tapeout不可逆 |
| NFR12: 整体数据可靠性 | 非Golden数据丢失率 < 0.01% | 注入CAS chunk损坏，统计可恢复比例；运行6个月统计实际丢失事件 | 成功标准 |
| NFR13: 冗余备份 | Golden数据至少存2份于不同物理位置 | 检查Golden数据副本分布，验证不同NAS路径/不同物理存储；定期校验副本一致性 | 增强归档可靠性 |
...
| NFR | 指标 | 测量方法 | 依据 |
|-----|------|---------|------|
| NFR18: Python API可用性 | 提供Python模块，核心操作可通过Python API调用 | 编写Python脚本调用全部核心API（commit/checkout/log/branch），验证功能与CLI等价 | 与pds_xxx系统集成 |
| NFR19: 子进程调用可靠性 | 流水线step作为子进程执行时，退出码和标准输出/错误可准确捕获 | 编写测试step（正常退出0、异常退出非0、大量输出），验证BIG准确捕获退出码和全部输出 | 子进程编排器模型 |
| NFR20: 路径透明性 | BIG管理的文件在NAS上的真实路径可被外部工具直接访问 | 验证EDA工具通过NAS路径直接读取BIG管理文件，比较读取内容与BIG checkout结果一致 | EDA工具无需感知BIG |
